    ...
    public class EntityWithListOfValueTypesPersistenceTests
    {
        ...
        public void UpdatingCollectionElementIsAudited()
        {
            using (var session = db.CreateSession())
            {
                ...
                var auditedEntities = session.Query<EntityWithListOfValueTypesAuditHistory>().Where(h => h.Id == 42).ToList();
        ...
        public void DeletingOwnerEntityWithoutLoadingLazyCollectionIsAudited()
        {
            using (var session = db.CreateSession())
            {
                ...
                session.Clear();
                entity = session.Get<EntityWithListOfValueTypes>(entity.Id);
        ...
        private void Configure(Configuration cfg)
        {
            ...
            var auditEntryFactory = new SimpleAuditEntryFactoryBuilder()
                .MapEntity<EntityWithListOfValueTypes, EntityWithListOfValueTypesAuditHistory>(x => new EntityWithListOfValueTypesAuditHistory { Id = x.Id })
                .MapComponent<ComponentType, ComponentType>(x => new ComponentType { Integer = x.Integer, String = x.String })
                .Build();
