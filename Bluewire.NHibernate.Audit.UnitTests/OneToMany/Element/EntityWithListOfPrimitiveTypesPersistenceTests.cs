    ...
    public class EntityWithListOfPrimitiveTypesPersistenceTests
    {
        ...
        public void UpdatingCollectionElementIsAudited()
        {
            using (var session = db.CreateSession())
            {
                ...
                var auditedEntities = session.Query<EntityWithListOfPrimitiveTypesAuditHistory>().Where(h => h.Id == 42).ToList();
        ...
        private void Configure(Configuration cfg)
        {
            ...
            var auditEntryFactory = new SimpleAuditEntryFactoryBuilder()
                .MapEntity<EntityWithListOfPrimitiveTypes, EntityWithListOfPrimitiveTypesAuditHistory>(x => new EntityWithListOfPrimitiveTypesAuditHistory { Id = x.Id })
                .Build();
