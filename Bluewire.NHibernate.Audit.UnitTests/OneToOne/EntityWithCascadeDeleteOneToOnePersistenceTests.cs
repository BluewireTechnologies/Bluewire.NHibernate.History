...
{
    [TestFixture]
    public class EntityWithCascadeDeleteOneToOnePersistenceTests
    {
        private TemporaryDatabase db;
        private MockClock clock = new MockClock();

        public EntityWithCascadeDeleteOneToOnePersistenceTests()
        {
        ...
        public void SavingEntityIsAudited()
        {
            ...
            {
                var entity = new EntityWithOneToOne { Id = 42, Reference = new ChildEntity { Value = 2 } };
                session.Save(entity);
                session.Flush();

                var audited = session.Query<EntityWithOneToOneAuditHistory>().Single(h => h.Id == 42);
        ...
        public void SavingEntityWithNoReferenceIsAudited()
        {
            using (var session = db.CreateSession())
            {
                var entity = new EntityWithOneToOne { Id = 42, Reference = null };
                session.Save(entity);
                session.Flush();
        ...
        [Test]
        public void RemovingChildEntityIsNotAudited()
        {
            using (var session = db.CreateSession())
            {
                var entity = new EntityWithOneToOne { Id = 42, Reference = new ChildEntity { Value = 2 } };
                ...
                session.Flush();
                Assume.That(entity.VersionId, Is.EqualTo(initialVersion));

                var audited = session.Query<EntityWithOneToOneAuditHistory>().Single(h => h.Id == 42 && h.VersionId == entity.VersionId);
        ...
        [Test]
        public void UpdatingReferencedEntityIsNotAudited()
        {
            using (var session = db.CreateSession())
            {
                var entity = new EntityWithOneToOne { Id = 42, Reference = new ChildEntity { Value = 2 } };
        ...
        public void DeletingSimpleEntityIsAudited()
        {
            using (var session = db.CreateSession())
            {
                ...
                var audited = session.Query<EntityWithOneToOneAuditHistory>().Where(h => h.Id == 42).ToList();
        ...
        private void Configure(Configuration cfg)
        {
            ...
            mapper.Class<ChildEntity>(e =>
            {
                e.Id(i => i.Id, i => i.Generator(Generators.Foreign<ChildEntity>(x => x.__Owner)));
                e.Property(i => i.Value);
                e.OneToOne(i => i.__Owner, x => x.Constrained(true));
            });
            mapper.Class<EntityWithOneToOne>(e =>
            {
            ...
            mapper.Class<EntityWithOneToOneAuditHistory>(e =>
            {
